...
Locator: locator('h2:has-text("FAILED_PUSH_TEST")')
Expected: visible
Timeout: 1000ms
...
Call log:
  ...
  - waiting for locator('h2:has-text("FAILED_PUSH_TEST")')
...
```ts
  ...
  10 |     // INTENTIONAL FAILURE FOR PUSH TEST
> 11 |     await expect(page.locator('h2:has-text("FAILED_PUSH_TEST")')).toBeVisible({ timeout: 1000 });
     |                                                                   ^ Error: expect(locator).toBeVisible() failed
  12 | 
